    ...
    public function oldCustomerHelp($val = null)
{
    ...
    public function propertyProduct(Request $request)
    {
        ...
        $sale = Sale::where('project_id',$request->project_id)->whereNotNull('file_type')->pluck('product_id')->unique()->toArray();

        $data = [];
        $product = Product::with('supplier','buyable_type')->where('product_form_type', $request->product_form_type)->where('status', 1);
        if(!empty($val)){
            $val = (string)$val;
